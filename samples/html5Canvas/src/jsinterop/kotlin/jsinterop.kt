...
external public fun stringLength(message: String): Int
...
fun runLambda(userData: Int, arena: Arena, arenaSize: Int) {
    ...
    println("Want to run lambda $userData ()");
...
open class JsValue(val arena: Arena, val index: Object) {
    fun getInt(property: String): Int {
        return getInt(arena, index, stringPointer(property), stringLength(property))
    }
}
...
fun setter(obj: JsValue, property: String, lambda: (arguments: ArrayList<JsValue>) -> Unit) {
    ...
    setFunction(obj.arena, obj.index, stringPointer(property), stringLength(property), index)
}
